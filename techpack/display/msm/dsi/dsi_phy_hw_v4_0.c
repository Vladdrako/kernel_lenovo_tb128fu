			    ...
			    struct dsi_phy_cfg *cfg)
{
	...
	if (phy->version == DSI_PHY_VERSION_4_1) {
		...
		if(cfg->dsi_phy_rescode_top >= 0)
		{
			glbl_rescode_top_ctrl = less_than_1500_mhz ? 0x3d :  cfg->dsi_phy_rescode_top;
		}
		if(cfg->dsi_phy_rescode_bot >= 0)
		{
			glbl_rescode_bot_ctrl = less_than_1500_mhz ? 0x39 :  cfg->dsi_phy_rescode_bot;
		}
